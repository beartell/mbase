...
InfProcessor::flags InfProcessor::unregister_client(InfClient& in_client)
{
	...
	mClientsMutex.acquire();
	llama_kv_cache_seq_rm(mModelContext, in_client.mSequenceId, -1, -1);
	mRegisteredClients.erase(in_client.mfrSelfIter);
	in_client._reset_client();
	mClientsMutex.release();

	in_client.on_unregister();
	return flags::INF_PROC_SUCCESS;
}
...
GENERIC InfProcessor::update_t()
{
	...
	for (client_list::iterator It = mRegisteredClients.begin(); It != mRegisteredClients.end(); ++It)
	{
		...
		else
		{
			...
			{
				I32 resultLength = llama_token_to_piece(get_processed_model()->get_raw_model(), generatedToken, myClient->mfrGeneratedToken.get_buffer(), 128, 0, false);
