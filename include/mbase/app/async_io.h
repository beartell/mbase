...
public:
	...
	MBASE_INLINE mbase::list<async_io_context*>* GetWriteContextList() noexcept;
	MBASE_INLINE mbase::list<async_io_context*>* GetReadContextList() noexcept;
	MBASE_INLINE U32 GetAllowedWriteContext() const noexcept;
	MBASE_INLINE U32 GetAllowedReadContext() const noexcept;
...
MBASE_INLINE mbase::list<async_io_context*>* async_io_manager::GetWriteContextList() noexcept 
{
	return &writeContextList;
}

MBASE_INLINE mbase::list<async_io_context*>* async_io_manager::GetReadContextList() noexcept 
{
	return &readContextList;
}

MBASE_INLINE U32 async_io_manager::GetAllowedWriteContext() const noexcept
{
	return maximumAllowedWriteContext;
}

MBASE_INLINE U32 async_io_manager::GetAllowedReadContext() const noexcept
{
...
MBASE_INLINE async_io_manager::flags async_io_manager::EnqueueWriteContext(async_io_context* in_ctx, size_type in_bytes_on_each) noexcept 
{
	...
	if (in_ctx->ais == async_io_context::flags::ASYNC_IO_STAT_IDLE || in_ctx->ais == async_io_context::flags::ASYNC_IO_STAT_OPERATING)
	{
...
MBASE_INLINE async_io_manager::flags async_io_manager::EnqueueReadContext(async_io_context* in_ctx, size_type in_bytes_on_each) noexcept 
{
	...
	if (in_ctx->GetIoDirection() != async_io_context::flags::IO_CTX_DIRECTION_INPUT)
	{
		return flags::AIO_MNG_ERR_INVALID_IO_DIRECTION;
	}
...
MBASE_INLINE GENERIC async_io_manager::ClearReadContexts() noexcept 
{
	...
	for (mbase::list<async_io_context*>::iterator It = readContextList.begin(); It != readContextList.end(); It++)
	{
		...
		iCtx->FlushContext();
		if (st != async_io_context::flags::ASYNC_IO_STAT_FINISHED)
		{
			iCtx->ais = async_io_context::flags::ASYNC_IO_STAT_ABANDONED;
		}
...
MBASE_INLINE GENERIC async_io_manager::RunWriteContexts() noexcept 
{
	MBASE_TS_LOCK(writeQueueMutex)
		...
		for (mbase::list<async_io_context*>::iterator It = writeContextList.begin(); It != writeContextList.end(); It++)
		{
			...
			iCtx->isActive = true;
			iCtx->ais = async_io_context::flags::ASYNC_IO_STAT_OPERATING;
			...
			{
				iCtx->isActive = false;
				iCtx->ais = async_io_context::flags::ASYNC_IO_STAT_FINISHED;
			...
			if (iCtx->ais == async_io_context::flags::ASYNC_IO_STAT_FINISHED)
			{
				iCtx->FlushContext();
				It = writeContextList.erase(It);
			}
			else
			{
				if (!writtenBytes)
				{
					...
					iCtx->ais = async_io_context::flags::ASYNC_IO_STAT_FAILED;
					iCtx->HaltContext();
					It = writeContextList.erase(It);
				}
				else
				{
					iCtx->ais = async_io_context::flags::ASYNC_IO_STAT_IDLE;
				}
			}
		}
